"""
Main proposal: CA-PFedAvg + ISL-Gossip (flat-state PPO, FedPer).
Run: python -m sim.train
"""
...
LOG_EVERY = 100; SAVE_DIR = Path("results/frl_isl")
...
def run():
    ...
    metrics = {"round": [], "mean_reward": [], "loss": [], "contact_size": [], "direct_size": [],
    ...
    for k in range(K_ROUNDS):
        ...
        metrics["direct_size"].append(len(direct))
        ...
        if k % 500 == 0:
            ...
            torch.save(agents[0].get_trunk_weights(), SAVE_DIR / ("trunk_%d.pt" % k))

    with open(SAVE_DIR / "metrics.json", "w") as f: json.dump(metrics, f)
    print("CA-PFedAvg+ISL-Gossip done. Saved to", SAVE_DIR)
