"""Ind-PPO baseline — no federation, same 10 steps/round."""
...
ACTION_DIM      = 4
LOG_EVERY       = 100
SAVE_DIR        = Path("results/indppo_isl")
...
def run():
    ...
    for ag in agents:
        ag.set_weights(init_w)

    x_prev     = np.zeros((N_SAT, N_SAT))
    kappa_prev = np.zeros((N_SAT, N_SAT))
    metrics    = {"round": [], "mean_reward": [], "loss": [],
    ...
    for k in range(K_ROUNDS):
        ...
        mean_r = float(np.mean(round_rewards))
        mean_l = float(np.mean(losses))
        metrics["round"].append(k)
        metrics["mean_reward"].append(mean_r)
        metrics["loss"].append(mean_l)
        ...
        if k % LOG_EVERY == 0:
            print("Round %4d | reward %+.4f | loss %.4f | kappa %.3f | drop %.3f | util %.3f | tput %.2fG" % (
                k, mean_r, mean_l, metrics["mean_kappa"][-1],
    ...
    with open(SAVE_DIR / "metrics.json", "w") as f:
        json.dump(metrics, f)
    print("Ind-PPO done. Saved to", SAVE_DIR)
